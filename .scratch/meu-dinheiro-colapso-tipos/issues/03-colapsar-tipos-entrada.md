...
- [ ] `main/domain/income.ts` apagado; `IncomeEntity` deixa de existir.
- [ ] `main/controllers/responses/income.response.ts` apagado;
      `incomeToResponse` deixa de existir.
- [ ] `incomesRepository.ts`: `rowToIncome` e todos os métodos do
...
- [ ] `incomesController.ts` chama o service direto, sem `incomeToResponse`.
- [ ] `incomesService.ts` (e qualquer outro consumidor) atualizado para o
      novo tipo — usar o `tsc` para achar todos os call sites.
- [ ] `main/domain/month.ts`: `MonthDetailEntity.incomes` referencia
