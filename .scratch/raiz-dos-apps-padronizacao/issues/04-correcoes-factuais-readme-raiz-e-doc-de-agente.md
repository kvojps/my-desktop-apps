...
- [x] README raiz §3.2: adiciona a linha dos builds Linux — `npm run
  dist:linux:<app>` gera o pacote `.deb` — paralela à linha do instalador
  Windows NSIS.
- [x] README raiz §3.3: o bullet do `tsconfig.base.json` passa a dizer que cada
  `apps/*/tsconfig.json` só faz `extends` dele — `paths` e `include` moram na
  base — em vez de "declara apenas os seus `paths`".
- [x] README raiz §4.1: o texto passa a dizer que todos os apps geram o aviso de
...
- [x] `docs/agents/domain.md`: a frase que lista `git-dlog`, `meu-negocio-app` e
  `meu-movel-planejado` como os apps com `CONTEXT.md` passa a listar os quatro
  (o `meu-dinheiro-app` também tem, e o `CONTEXT-MAP.md` já o lista). O diagrama
  de árvore da mesma seção também passa a mostrar `meu-dinheiro-app/CONTEXT.md`.
- [x] `npm run format:check` limpo.
- [x] Nenhuma mudança de comportamento; nenhum `README.md`/`CONTEXT.md` de app é
  tocado.
