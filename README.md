...
`HashRouter` (necessário no `file://` do build), imports por alias (`@/` e
`@shared/`) sempre que saírem da própria pasta. Os apps não migrados usam MUI com
...
Os demais apps mantêm MUI e Material Icons; não há compartilhamento de código.
