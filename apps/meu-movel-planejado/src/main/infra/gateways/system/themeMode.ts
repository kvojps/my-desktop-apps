...
/**
 * O que o `settingsService` precisa do tema, e só isso — um duble de teste para
 * `setThemeMode` implementa `apply`, não a superfície inteira.
 */
 ...
 * peça cor de fundo nem a preferência do SO.
 */
...
export const themeMode: ThemeModeSystemGateway = {
  ...
  /**
   * A fonte é o próprio `nativeTheme`: `apply` o fixa em `light`/`dark` e nunca o
   * deixa em `'system'`, então ler de volta é ler a última escolha aplicada — sem
   * uma segunda cópia de estado para dessincronizar. Só responde certo depois do
   * primeiro `apply` (o do bootstrap).
   */
  currentMode(): ThemeModeEntity {
   ...
   * `'system'`, ou seja **antes** do primeiro `apply` — por isso o bootstrap lê
   * uma vez só, na resolução inicial, e toda leitura posterior passa por
   * `currentMode`, não por aqui.
   */
  systemPrefersDark(): boolean {
