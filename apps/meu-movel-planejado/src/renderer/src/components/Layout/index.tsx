...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh' }}>
      ...
      >
        <Tooltip title="Meu Móvel Planejado" placement="right">
          <Box
            component="img"
            src={logo}
            alt="Meu Móvel Planejado"
            ...
            <Tooltip key={item.path} title={item.label} placement="right">
              <ButtonBase
                ...
                sx={{
                  ...
                  // A mesma tinta da linha sob o cursor: item ativo do rail é
                  // `tint(0.1)` (design system, §2), e não um alpha próprio.
                  bgcolor: (theme) => (active ? tint(theme.palette.mode, 0.1) : 'transparent'),
                  transition: (theme) =>
                  ...
                  // O item ativo precisa dos dois canais: sem rótulo, a cor
                  // seria o único sinal, e cor sozinha não basta. Esta barra
                  // encosta na borda esquerda do rail e sobrevive a daltonismo
                  // e a alto contraste.
                  ...(active && {
                    '&::before': {
                      content: '""',
      ...
      <Box component="main" sx={{ flex: 1, minWidth: 0, overflowY: 'auto' }}>
        {/* Em monitor largo o conteúdo para de crescer e centraliza: sem teto, o
            ...
            ~128px. Este container nomeado deixa os componentes consultarem a
            largura que realmente têm, via `@container content (min-width: …)`.
